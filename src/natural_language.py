...
def sort_words():
    ...
    ans1.close();
    '''
    ans2 = open('./test/data/tongji.txt', 'w', encoding='utf-8')
    for i in words:
        s=0
        if i in word_neg:
            s=s+word_neg[i]
...
#读入评论数据
def start():
    load('./test/data/chen.xls')
    #load('./test/data/du.xls')
    #load('./test/data/guo.xls')
    #load('./test/data/yang.xls')
#读入停用词
    data_stop_word=open('./test/data/stop_word.txt','r',encoding='utf-8')
#把停用词读入list
    while True:
        ...
        if i:
            ...
            break
    stop_word_test=open('./test/data/test_stopword.txt', 'w', encoding='utf-8')
    for i in stop_word:
